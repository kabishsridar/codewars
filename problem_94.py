...
def anagrams(subjects, memories, players):
    ...
    round_index = 0
    subject_index = 0
    max_rounds = len(subjects)
    order = [(0, 2), (2, 1), (1, 3), (3, 0),
             (0, 3), (2, 0), (1, 2), (3, 1)]

    start = 0
    while start < 8:
        if order[start] == tuple(players):
            break
        start += 1

    while subject_index < max_rounds:
        first, second = order[start]
        subject = subjects[subject_index]
        sorted_subject = sorted(subject)

        memory_first = memories[first]
        memory_second = memories[second]

        first_points = 0
        if subject in memory_first:
            first_points += 1

        i = 0
        first_anagrams = []
        while i < len(memory_first):
            word = memory_first[i]
            if word != subject and sorted(word) == sorted_subject:
                first_anagrams.append(word)
                first_points += 2
            i += 1

        second_points = 0
        if subject in memory_second:
            second_points += 1

        i = 0
        while i < len(memory_second):
            word = memory_second[i]
            if word != subject and sorted(word) == sorted_subject:
                if word not in first_anagrams:
                    second_points += 3
            i += 1
