...
class Plugboard(object):
    def __init__(self, wires=""):
        self.wiring = wires
        if len(self.wiring) % 2 != 0:
            raise ValueError("Wiring must consist of even number of characters.")
        if len(set(self.wiring)) != len(self.wiring):
            raise ValueError("Each letter should be used only once.")
        if len(self.wiring) > 20:
            raise ValueError("Maximum of 10 wire pairs (20 letters) are allowed.")

        self.mapping = {}
        for i in range(0, len(self.wiring), 2):
            a, b = self.wiring[i], self.wiring[i + 1]
            if not (a.isalpha() and b.isalpha() and a.isupper() and b.isupper()):
                raise ValueError("Wiring must only contain uppercase A-Z letters.")
            self.mapping[a] = b
            self.mapping[b] = a
